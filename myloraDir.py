...
#
class myNode():
    def __init__(self, nodeid, bs, period, packetlen, distribution, dataPattren):
        self.nodeid = nodeid
        self.period = period
        self.bs = bs
        self.x = 0
        ...
        # Omer added code
        self.my_var = 1 ;
        self.meanRate = period
        self.pktLength = packetlen
        self.dataDistribution = distribution 
        self.pattren = dataPattren 
...
def transmit(env,node):
    #Omer Declared Variables

    
    while True:
        #if experiment == 1:
            #node.packet.freq = random.choice([860000000, 864000000, 868000000, 872000000, 876000000])
        #yield env.timeout(random.expovariate(1.0/float(node.period)))
        #yield env.timeout(((random.randint(1,290)))*1000)
                                 #yield env.timeout((node.nodeid + 1)*1000) 
                         
                     ########### I NEED TO TAKE UP FROM HERE ###########
        #yield env.timeout(((node.nodeid + 1)*1000)%8640000)
        # time sending and receiving
        # packet arrives -> add to base station
        
        
        if node.pattren == "-P" or node.pattren == "-p":
            yield env.timeout(node.meanRate)
            yield env.timeout(((random.randint(10, 100)))*1000)
            for iterator in range (0, numberOfApplications):
                if appIdentifier[iterator] == node.dataDistribution:
                    appSent[iterator] = appSent[iterator] + 1
                    break
        elif node.pattren == "-E" or node.pattren == "-e":
            yield env.timeout(random.expovariate(1.0/float(node.meanRate)))
            for iterator in range (0, numberOfApplications):
                if appIdentifier[iterator] == node.dataDistribution:
                    appSent[iterator] = appSent[iterator] + 1
                    break
        elif node.pattren == "-R" or node.pattren == "-r":
            yield env.timeout(random.randint(0, node.meanRate))
            for iterator in range (0, numberOfApplications):
                if appIdentifier[iterator] == node.dataDistribution:
                    appSent[iterator] = appSent[iterator] + 1
                    break
        ...
        if node.packet.collided == 0 and not node.packet.lost:
            ...
            for iterator in range (0, numberOfApplications):
                if appIdentifier[iterator] == node.dataDistribution:
                    appReceived[iterator] = appReceived[iterator] + 1
                    break
...
#print('Simulation start time =%d'%env.now) 
...
if len(sys.argv) < 4:
    print("Error in command line argument list") 
    exit(-1)
    
experiment = int(sys.argv[2]) 
simtime = int(sys.argv[3])
if len(sys.argv) > 4:
    full_collision = bool(int(sys.argv[4]))
    #print "Nodes:", nrNodes
...
if len(sys.argv) >= 4:
    ...
    if not inputFileName:
        ...
        exit(-1) 
    with open(inputFileName, "rb") as trafficModel:
        while True:
            parameters = trafficModel.readline() 
            if not parameters: break
            values = parameters.split()
            if len(values) != 4:
                print("Wrong number of parameteres in input file")
                exit(-1)
            else:
                for i in range(0, int(values[0])):
                    node = myNode(nodeNo,bsId, int(values[2]), int(values[3]), (values[1]+values[2]+values[3]), values[1])
                    nodes.append(node)
                    env.process(transmit(env,node))
                    nodeNo = nodeNo + 1
            appIdentifier.append(values[1] + values[2] + values[3])
            print(appIdentifier[numberOfApplications])
            numberOfApplications = numberOfApplications + 1
            
for x in range(0, numberOfApplications):
    appSent.append(0)
    appReceived.append(0) 

for x in range(0, len(appSent)):
    print(appSent[x])
    print(appReceived[x]) 
...
for iterator in range (0, numberOfApplications):
    print "DER Applicationt ", iterator+1, (float(appReceived[iterator])/float(appSent[iterator]))
    #print "Sent Application ", iterator, appSent[iterator]
    #print "Received Application ", iterator, appReceived[iterator]
print "Total Energy Consumed (in J): ", energy
print '{:50}'.format("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
...
#     bfile.write("{} {} {}\n".format(bsx, bsy, 0))
#print('Simulation End time =%d'%env.now)
